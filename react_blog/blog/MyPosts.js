...
class MyPosts extends Component {
    ...
    deletePost(){
         axios.delete('/api/me/posts/'+this.state.deleted+'/deleted').
         then((response)=>{this.setState({myposts:response.data.myposts});
        })    
    }
  componentWillMount(){
                      ...
                      {this.state.myposts.map((value, index) => {
                        return <div key = {index} >
                                <ul className='post_li'>
                                  <li className="list-group-item post_cat">{value.category.title}</li>
                                  ...
                                  <li className="btn  btn-info post_edit" ><Link to ={'my-posts/'+value.id+'/edit'}  id='ed' >Edit</Link></li>
                                </ul>
                               </div>           
                          })}
                </div> 
            ...
            <div className="modal-dialog" role="document">
                <div className="modal-content">
                    <div className="modal-header">
                      <h5 className="modal-title" id="exampleModalLabel" style={{textAlign:'center'}}>Are you sure you want to delete the Post</h5>
